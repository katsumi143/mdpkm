...
import { TabItem, Typography } from 'voxeliface';
...
export default function Content({ instance }: ContentProps) {
    ...
    return <React.Fragment>
		...
		>
			<TabItem name={t('mod_management.tab.0')} icon={<IconBiList/>} value={0} disabled={!useModSearch}>
				<ModManagement instance={instance}/>
			</TabItem>
			<TabItem name={t('mod_management.tab.1')} icon={<IconBiSearch fontSize={11}/>} value={1} disabled={!useModManagement}>
				<ModSearch onClose={() => setTab(0)} instance={instance}/>
			</TabItem>
			<TabItem name={t('mod_management.tab.2')} icon={<IconBiDiamond/>} value={2} spacing={4} disabled={!useEssential}>
