...
import { Diamond, ArrowClockwise, CloudArrowDown } from 'react-bootstrap-icons';
...
export default Patcher.register(function ModManagement({ instanceId }) {
    ...
    return <React.Fragment>
        <Grid margin="4px 0" spacing={8} justifyContent="space-between">
            ...
            <Grid spacing={8} justifyContent="end">
                <TextInput
                    width={144}
                    value={filter}
                    onChange={setFilter}
                ...
                <Dialog.Root >
                    <Dialog.Trigger asChild>
                        <Button>
                            <Plus size={16} />
                            {t('app.mdpkm.instance_page.tabs.mods.tabs.search')}
                        </Button>
                    </Dialog.Trigger>
                    <Dialog.Content css={{ maxWidth: "90vw", height: "100%" }}>
                        <Grid direction="vertical" css={{ height: "100%" }} spacing={8}>
                            <Dialog.Title>
                                <Grid spacing={8} justifyContent="space-between">
                                    ...
                                    <Dialog.Close asChild>
                                        <Button theme="accent">
                                            <X size={16} />
                                            {t('app.mdpkm.common:actions.close')}
                                        </Button>
                                    </Dialog.Close>
                                </Grid>
                            </Dialog.Title>
                            <Tabs value={tab} onChange={event => setTab(event.target.value)} borderRadius={0} css={{ flex: "1 1 auto", }}>
                                <TabItem name={t('app.mdpkm.instance_page.tabs.mods.tabs.search')} value={0}>
                                    <ModSearch instanceId={instanceId} />
                                </TabItem>
                                <TabItem name={t('app.mdpkm.instance_page.tabs.essential')} icon={<Diamond size={14} />} value={1} spacing={4}>
